...
var hImages = /([\s\S]*?src=")(([^"]+))(?!\s*[;,]?\s*\/\*\s*base64:skip\s*\*\/)|([\s\S]+)/img;
...
module.exports = (function () {
   ...
   * encodes any images it finds.
   *
   * @param srcFile Relative or absolute path to a source HTML file.
   * @param opts Options object
   * @param done Function to call once encoding has finished.
   */
  exports.html = function(file, opts, done) {
    opts = opts || {};
    ...
    function(complete) {
      // if there is another url to be processed, then:
      //    group[1] will hold everything up to the tag source
      //    group[2] will be the same as group[3] (useful if no encoding will take place) just copy the mode in stylesheet
      //    group[3] will hold the contents of the source
      //    group[4] will be undefined
      // if there is no other url to be processed, then group[1-3] will be undefined
      //    group[4] will hold the entire string
